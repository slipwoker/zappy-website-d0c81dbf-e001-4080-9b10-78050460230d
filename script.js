document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const getErrorMessage = function (field, rule, param) {
    ...
    const validateField = function (field) {
      ...
      if (field.dataset.minlength) {
        if (!validators.minLength(value, field.dataset.minlength)) {
          showError(field, getErrorMessage(field, 'minLength', field.dataset.minlength));
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .catch(function () {
          const errorMessage = contactForm.querySelector('.form-error-general');
          if (errorMessage) {
  ...
  const OBSERVED_SELECTORS = '.fade-in, .animate-on-scroll, [data-animate]';
  const animatableElements = document.querySelectorAll(OBSERVED_SELECTORS);
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "יעל טומשין אדריכלות ועיצוב משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: 'd0c81dbf-e001-4080-9b10-78050460230d',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || 'd0c81dbf-e001-4080-9b10-78050460230d';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = 'd0c81dbf-e001-4080-9b10-78050460230d';
